...
define(function (require) {
    ...
    var reads = svg.append("g")
        .attr("class", "reads")
        .attr("transform", "translate(" + utils.marginSignalGraph.left + "," + (utils.marginSignalGraph.top + (utils.height-200) + utils.heightminiSignalGraph) + ")")
        .call(rXZoom);
    ...
    d3.json("static/includes/app/data/events.json", function(error, data) {

        if (error) throw error;

        console.log("Event's Length: " + data.length);
